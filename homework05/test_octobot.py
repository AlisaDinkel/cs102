...
class TestOctobot(unittest.TestCase):
    def test_is_valid_date(self):
        ...
        self.assertFalse(octobot.is_valid_date("11/10/24", "/"))
        self.assertTrue(octobot.is_valid_date("09/10/24", "/"))
        self.assertFalse(octobot.is_valid_date("01/01/23", "/"))
        self.assertFalse(octobot.is_valid_date("04/05/23", "."))
        self.assertFalse(octobot.is_valid_date("09/04.24", "."))
        self.assertTrue(octobot.is_valid_date("02/12/23", "/"))
        today = datetime.today().date().strftime("%d/%m/%y")
        self.assertTrue(octobot.is_valid_date(today, "/"))
        self.assertFalse(octobot.is_valid_date("05/09/30", "/"))
        self.assertTrue(octobot.is_valid_date("10.10.23", "."))
        self.assertFalse(octobot.is_valid_date("35/04/24", "/"))
